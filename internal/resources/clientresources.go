...
func DefaultClientListener(target, routeName string) *v3listenerpb.Listener {
	hcm := utils.MarshalAny(&v3httppb.HttpConnectionManager{
	...
	if tlsContext != nil {
		ts = &v3corepb.TransportSocket{
			...
			ConfigType: &v3corepb.TransportSocket_TypedConfig{
				TypedConfig: utils.MarshalAny(tlsContext),
			},
		}
	}
	return &v3listenerpb.Listener{
				...
				Filters: []*v3listenerpb.Filter{
					{
						...
						ConfigType: &v3listenerpb.Filter_TypedConfig{
							TypedConfig: utils.MarshalAny(&v3httppb.HttpConnectionManager{
...
func HTTPFilter(name string, config proto.Message) *v3httppb.HttpFilter {
	return &v3httppb.HttpFilter{
		...
		ConfigType: &v3httppb.HttpFilter_TypedConfig{
			TypedConfig: utils.MarshalAny(config),
		},
	}
}
